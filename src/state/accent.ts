 ...
 *
 * The stylesheet's own `--accent` is the default, and this only ever overrides
 * it as an inline property on the root element. With nothing stored, or with a
 * value that is not on the list any more, the page keeps what the stylesheet
 * says — see styles.css.
 */
...
function apply(): void {
  document.documentElement.style.setProperty('--accent', resolveAccent(settings.current().accent))
}
